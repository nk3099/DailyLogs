...
router = APIRouter(
    ...
    tags=['Posts'] #tags used to group the routes in the OpenAPI documentation
...
def get_posts(conn = Depends(get_db)):  #conn is a dependency that gets the database connection
    cursor = conn.cursor()
...
def create_posts(payload: schemas.validate_postCreate, conn = Depends(get_db)):
   cursor = conn.cursor()  #get the cursor from the database connection
...
def get_post(id : int, conn = Depends(get_db)): #Type hint tells FastAPI to validate the input
   cursor = conn.cursor()
...
def update_post(id:int, post: schemas.validate_postCreate, conn = Depends(get_db)):

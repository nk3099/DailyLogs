...
@app.post("/posts", status_code = status.HTTP_201_CREATED, response_model = schemas.postResponse) #status code 201 Created
...
@app.get("/posts/{id}", response_model=schemas.postResponse) #id -> path parameter in the URL, note: it always gets returned as a string, therefore we convert it to int
